...
const char* kpath = "benchmarks/gaussian2/gaussian_kernel.cu";

int Size;
float *a_cuda, *b_cuda, *finalVec;
float *m_cuda;
...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    for(i=1;i<argc;i++) {
      if (argv[i][0]=='-') {// flag
        flag = argv[i][1];
          ...
          slateMalloc((void **) &a_cuda, Size * Size * sizeof(float));
	
	      create_matrix(a_cuda, Size);

	      //b = (float *) malloc(Size * sizeof(float));
          slateMalloc((void **) &b_cuda, Size * sizeof(float));	
	      for (j =0; j< Size; j++)
	    	b_cuda[j]=1.0;

	      //m = (float *) malloc(Size * Size * sizeof(float));
          slateMalloc((void **) &m_cuda, Size * Size * sizeof(float));
              break;
            case 'f': // platform
              i++;
    ...
    if (verbose) {
        printf("Matrix m is: \n");
        PrintMat(m_cuda, Size, Size);

        printf("Matrix a is: \n");
        PrintMat(a_cuda, Size, Size);

        printf("Array b is: \n");
        PrintAry(b_cuda, Size);
    }
    BackSub();
    if (verbose) {
...
void InitProblemOnce(char *filename)
{
	...
	InitMat(a_cuda, Size, Size);
	//printf("The input matrix a is:\n");
	//PrintMat(a, Size, Size);
    slateMalloc((void **) &b_cuda, Size * Size * sizeof(float));
...
clock_gettime(CLOCK_MONOTONIC, &start);
	for (t=0; t<(Size-1); t++) {
	//for (t=0; t<1000; t++) {
		slateLaunchKernel(kpath, "Fan1", dimGrid, dimBlock, 0, m_cuda,a_cuda,Size,t);
		slateLaunchKernel(kpath, "Fan2", dimGridXY,dimBlockXY, 0, m_cuda,a_cuda,b_cuda,Size,Size-t,t);
	}
    //printf("kernels subbed\n");
	// end timing kernels
	struct timeval time_end;
    gettimeofday(&time_end, NULL);
...
void BackSub()
{
	...
	int i,j;
	for(i=0;i<Size;i++){
		finalVec[Size-i-1]=b_cuda[Size-i-1];
		for(j=0;j<i;j++)
		{
			finalVec[Size-i-1]-=*(a_cuda+Size*(Size-i-1)+(Size-j-1)) * finalVec[Size-j-1];
		}
		finalVec[Size-i-1]=finalVec[Size-i-1]/ *(a_cuda+Size*(Size-i-1)+(Size-i-1));
	}
}

                                  ...
                                  stream(river), kernel_name(kname)
{
        ...
        linkandload(prog);
        auto end = Clock::now();
        std::cout << "compile time: " << std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() << " ns\n";
}
...
void SlateKernel::compile(const char* codepath, nvrtcProgram& prog)
{
    ...
    slate_yylex(final_kernel, lexer_state);
...
junk.close();
    ...
    CUDA_SAFE_CALL(cuModuleGetFunction(&kernel, module, "scheduler"));//kernel_name));
}
